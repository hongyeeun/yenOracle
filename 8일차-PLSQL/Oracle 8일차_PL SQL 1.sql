-- 8일차 PL/SQL
-- Oracles's Procedural Language Extension to SQL의 약자
...
-- IF (조건식) THEN (실행문1) ELSIF (조건식2) THEN (실행문2)
-- ELSE (실행문3) END IF;
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('급여 : '||SALARY*10000);    --다시곱해줘야함
...
BEGIN
    ...
    FROM EMPLOYEE
    WHERE EMP_ID = '&사번';
    SAL := SALARY;  -- SAL에 SALARY 대입하여 보존함
...
BEGIN
    LOOP
        ...
        -- 랜덤함수 : DBMS_RANDOM.VALUE(시작 값,끝값+1) -> 소수값으로 나온다.
        N := N + 1;
        EXIT WHEN N > 5;
    END LOOP;
END;
...
BEGIN
    ...
    LOOP
        ...
        -- 나머지가 0이면 짝수니까 출력하지 않고 반복으로
        -- EXIT 없어도 제어조건을 설정할 수 있음.
        DBMS_OUTPUT.PUT_LINE(N);
    END LOOP;
END;
/
...
-- ex) EXCEPTION WHEN NO_DATE_FOUND THEN DBMS_OUTPUT.PUT_LINE('노 데이터'!);
-- NO_DATA_FOUND
