...
CREATE OR REPLACE TRIGGER TRG_EMP_NEW -- OR REPLACE 옵션 가능하다
AFTER
INSERT ON EMPLOYEE
...
VALUES((SELECT MAX(EMP_ID)+1 FROM EMPLOYEE), '이용자', '000624-322344', '01028272733', 'J5', 'S5');
...
CREATE OR REPLACE TRIGGER TRG_INFO_REP
AFTER 
UPDATE ON EMPLOYEE
FOR EACH ROW
BEGIN 
    IF(:OLD.SALARY != :NEW.SALARY)  --EMPLOYEE에 UPDATE가 되면 무조건 실행되는 것을 방지
    THEN
        DBMS_OUTPUT.PUT_LINE('급여 정보가 변경되었습니다.');
        DBMS_OUTPUT.PUT_LINE('변경 전 급여: '||:OLD.SALARY); -- 변경 전의 레코드(OLD)
        DBMS_OUTPUT.PUT_LINE('변경 후 급여: '||:NEW.SALARY); -- 변경 후의 레코드(NEW)
    END IF;
END;
/

UPDATE EMPLOYEE
SET SALARY = '1000000' WHERE EMP_ID='224';
-- 급여 정보가 변경되었습니다.
-- 변경 전 급여: 3000000
-- 변경 후 급여: 2000000
COMMIT;
-- 정보가 나오지 않으면 COMMIT 해보기 (메모)
SELECT * FROM EMPLOYEE WHERE EMP_ID ='224';
-- 의사레코드 OLD, NEW
-- FOR EACH ROW를 사용
...
CREATE TABLE PRODUCT_IO(
    ...
    PCODE NUMBER, --CONSTRAINT FK_PRODUCT_IO REFERENCES PRODUCT(PCODE) -- 외래키참조하는 법1
    PDATE DATE,
    AMOUNT NUMBER,
    STATUS VARCHAR2(10) CHECK(STATUS IN ('입고', '출고')),
    CONSTRAINT FK_PRODUCT_IO FOREIGN KEY(PCODE) REFERENCES PRODUCT(PCODE) -- 외래키 참조2
);
...
NOCYCLE
NOCACHE;
-- CYCLE, CACHE 없는건 꼭 적어줘야한다. 나머진 안 적어도 기본값
...
FOR EACH ROW
BEGIN
    IF :NEW.STATUS = '입고'   --  새로운 값이 삽입되는데 그것이 입고상태일때
    THEN
    -- INSERT INTO PRODUCT_IO VALUES(1, 1, SYSDATE, 10, '입고'); 일때
        UPDATE PRODUCT SET STOCK = STOCK + :NEW.AMOUNT WHERE PCODE = :NEW.PCODE;
    ELSIF :NEW.STATUS ='출고' -- 새로운 값이 삽입되는데 그것이 출고상태일때
    THEN
    -- INSERT INTO PRODUCT_IO VALUES(2, 1, SYSDATE, 5, '출고'); 일때 
        UPDATE PRODUCT SET STOCK = STOCK - :NEW.AMOUNT WHERE PCODE = :NEW.PCODE;
    END IF;
END;
/
...
-- 6. 제품 춤고 테이블 INSERT -> 입고이면 재고에 +1, 출고이면 재고에 -1
