...
--  1.2.1 Stored View(저장o) - 이름을 붙여서 저장함.
    ...
    FROM EMPLOYEE;
-- AS 뒤에 SELECT문은 내가 볼 정보들(앞에서 했던 SELECT문들)
-- 주의 : VIEW 만들 때에는 권한이 필요함, 권한명 : CREATE VIEW
-- System 계정으로 권한 부여
...
-- VIEW 이름 뒤에 () 로 보여줄 칼럼의 이름을 정할 수 있다.
-- 칼럼을 그대로 쓸 수 있을 뿐만 아니라 연봉처럼 산술연산도 가능하다.
SELECT * FROM annual_salary_view;

-- ex) 전체 직원의 사번, 이름, 직급명, 부서명, 지역명을 
-- 볼 수 있는 VIEW를 생성하시오(ALL_INFO_VIEW)
...
-- 1.5 VIEW 옵션
-- 기존 VIEW에 (SALARY) 컬럼을 추가하여 만들고 싶다면?
-- 지우고 만들기
...
SELECT * FROM V_EMPLOYEE;
-- 1.5.1 OR REPLACE
-- 수정하기 (ALTER 대신 OR REPLACE 조건 사용)
...
-- * 실습할때 VIEW를 계속 수정해야하므로 ON REPLACE 로 먼저 생성해준다.
CREATE OR REPLACE VIEW V_EMP_INFO(사원아이디, 사원명, 직급명, 부서명, 관리자명, 입사일)
AS SELECT EMP_ID, EMP_NAME, JOB_NAME
, NVL(DEPT_TITLE,'미정')
, NVL((SELECT EMP_NAME FROM EMPLOYEE WHERE E.MANAGER_ID=EMP_ID),'없음'), HIRE_DATE
FROM EMPLOYEE E 
...
-- SELECT EMP_NAME FROM_EMPLOYEE WHERE EMP_ID = 송종기의매니저ID);
-- 해당사원(예시 "송종기)의매니저ID:E.MANAGER_ID
...
-- 데이터 딕셔너리 뷰의 종류
-- 1. USER_XXX
SELECT * FROM USER_VIEWS;   -- 내가 만든 뷰 조회
SELECT * FROM USER_CONSTRAINTS; -- 내가 만든 테이블과 뷰의 제약조건 조회
SELECT * FROM USER_SYS_PRIVS;   -- 현재 사용자에게 부여된 시스템 권한에 대한 정보
SELECT * FROM USER_ROLE_PRIVS;  -- 현재 사용자에게 부여된 역할(roles)에 대한 정보
...
-- 마지막 번호가 몇번이었는지 몇식 증가해서 들어가야하는지를 기억하고 있어야함.
-- 그런 역할을 하는 것이 바로바로 sequence임 (무조건 사용한다..!!!)

-- 2.1 개요
-- - 순차적으로 점수 값을 자동으로 생성하는 객체, 자동 번호 발생기(채번기)의
-- 역할을 함.
-- - 사용법 : CREATE SEQUENCE 시퀀스명; -> 기본값으로 생성(1부터 1씩 증가)

-- 2.2 Seqeunce 옵션
-- 2.2.1 MINVALUE : 발생시킬 최소값 지점
-- 2.2.2 MAXVALUE : 발생시킬 최대값 지점
...
SELECT SEQ_KH_USER_NO.NEXTVAL FROM DUAL; -- 첫 NEXTVAL은 시퀀스를 개시함. 필수조건 (누르면 증가된다)
SELECT SEQ_KH_USER_NO.CURRVAL FROM DUAL; -- 현재 값을 나타냄.
...
SELECT * FROM USER_SEQUENCES;   -- 바뀐 것 확인 가능
