-- 7일차 PL/SQL
-- Oracles' Prodedural Language Extension to SQL의 약자
-- 1. 개요
...
EXCEPTION   -- 예외처리 (선동이인 경우 데이터가 없으므로 NO DAPTA가 나옴)
...
-- 2. PL/SQL 변수
DESC EMPLOYEE;
DECLARE     -- 변수 지정할때 EMPLOYEE와 데이터타입 똑같이 해줘야함
    VEMPNO  EMPLOYEE.EMP_NO%TYPE;   --내가 직접 적을 필요 없이 TYPE을 갖고온다.
    VENAME  EMPLOYEE.EMP_NAME%TYPE;
    VSAL    EMPLOYEE.SALARY%TYPE;   
    VHDATE  EMPLOYEE.HIRE_DATE%TYPE;
BEGIN
    SELECT EMP_NO, EMP_NAME, SALARY, HIRE_DATE
    INTO VEMPNO, VENAME, VSAL, VHDATE   -- SELECT에 해당되는 값 넣어줌
    FROM EMPLOYEE
    WHERE EMP_ID = '200';
...
-- PL/SQL 입력 받기
DECLARE
    VEMP EMPLOYEE%ROWTYPE;  -- 전체를 받을 땐 ROWTPYE으로
BEGIN
    SELECT *    
    INTO VEMP
    FROM EMPLOYEE
...
BEGIN
    ...
    WHERE EMP_ID='&EMP_ID';
    -- 결과 출력
    DBMS_OUTPUT.PUT_LINE('사번: '||VEMPID||', 이름 :'||VENAME||
    ', 부서코드: '||VDCODE||', 부서명: '||VDTITLE);
END;
/
...
INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME, EMP_NO, PHONE, DEPT_CODE, SAL_LEVEL)
VALUES(LAST_NUM+1, '&EMP_NO', '&PHONE', '&DEPT_CODE', '&SAL_LEVEL');
...
BEGIN
    SELECT*
    INTO EMP_INFO -- 여러 정보를 담고 있다.
    FROM EMPLOYEE
    WHERE EMP_ID = '&EMP_ID';
    DBMS_OUTPUT.PUT_LINE('사번: '||EMP_INFO.EMP_ID);  --EMP_INFO에서 EMP_ID를 꺼내서 쓰겠다.
...
DECLARE
    ...
    JCODE JOB.JOB_CODE%TYPE;    --출력하지 않아도 JCODE를 사용하므로 선언해줘야한다.
BEGIN
    SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME, JOB_CODE --JOB_CODE 받아야한다.
    ...
    DBMS_OUTPUT.PUT_LINE('사번 : '||JNAME);
    
    IF(JCODE='J1')
    THEN DBMS_OUTPUT.PUT_LINE('소속 : 대표');
    ELSE DBMS_OUTPUT.PUT_LINE('소속 : 일반사원');
    END IF;
END;
/
...
BEGIN
    ...
    FROM EMPLOYEE
    WHERE EMP_ID = '&EMP_ID';
    SAL := FLOOR(SAL/1000000);  -- 앞의 숫자로만 판단 / CASE일땐 범위가 안되므로
    CASE SAL
        WHEN 0 THEN SLV := 'F';
        WHEN 1 THEN SLV := 'E';
        WHEN 2 THEN SLV := 'D';
