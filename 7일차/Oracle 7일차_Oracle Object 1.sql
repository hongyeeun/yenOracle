...
-- 프로시저(PROCEDUAL), 함수(FUNCTION), 트리거(TRIGGER), 동의어(SYSNONYM), 커서(CURSOR), ...
...
-- CREATE VIEW 권한이 RESOURCE ROLE 에 없었기 때문에 따로 권한부여를 해줌
...
-- 데이터의 변경작업이 자주 일어나는 테이블에 INDEX 생성 시 오히려 성능저하가 발생할 수 있음.
-- - 어떤 컬럼에 인덱스를 만들면 좋을까?
