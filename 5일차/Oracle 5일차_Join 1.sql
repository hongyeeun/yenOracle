...
-- 0 대신 9로 형식을 지정할 수 있다. 형변환 -> TO_CHAR
-- NVL(해당 값, 0) : 해당값이 NULL이면 대신 0으로 RETURN
FROM EMPLOYEE;
...
SELECT DECODE(ENT_YN, 'N', '재직', '퇴직') "상태" , COUNT(*) "직원 수"
FROM EMPLOYEE
GROUP BY ENT_YN;
-- DECODE는 IF THEN ELSE문이다. ENT_YN 이 N이면 재직, 아니면 퇴직
...
-- DEPARTMENT 와 JOIN했으므로, LOCATION 과 조인할때 DEPARTMENT의 칼럼을 사용가능하다
-- 따라서, JOIN 할 테이블의 순서가 중요하다.
...
SELECT EMP_NAME, JOB_NAME, SALARY, SALARY*12 "연봉" , MAX_SAL "최대급여"
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
...
-- 2개 빠진 이유, DEPT_CODE가 NULL인 데이터를 빠짐
-- -> 이것을 INNER JOIN 이라고 한다. (INNER 생략 가능)
SELECT COUNT (*) FROM EMPLOYEE; -- 전체 23개

-- LEFT OUTER JOIN은 왼쪽 테이블이 가지고 있는 모든 데이터를 출력
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
LEFT OUTER JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID; -- 23개임 데이터 안 빠짐

-- RIGHT OUTER JOIN은 오른쪽 테이블이 가지고 있는 모든 데이터를 출력
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
RIGHT OUTER JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID; -- 24개임

-- FULL OUTER JOIN은 양쪽 테이블이 가지고 있는 모든 데이터를 출력
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
FULL OUTER JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID; -- 26개임
